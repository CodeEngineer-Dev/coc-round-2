...
const { Renderer, RenderComponent } = (function () {
    ...
    class AssetManager {
        ...
        parseGLB(data) {
            ...
            const binary_chunk = {
                ...
                data: new Uint8Array(data, 20 + buffer.getUint32(12, true) + 8, buffer.getUint32(20 + buffer.getUint32(12, true), true)),
            };
        ...
        loadModel(name, json, binary) {
            ...
            if ('images' in json && 'samplers' in json && 'textures' in json) {
                ...
                const images = json.images.map((image) => {
                    ...
                    const imageData = binary.slice(bufferView.byteOffset, bufferView.byteOffset + bufferView.byteLength);
                    // Must be PNG images
                ...
                // Clone samplers.
                const samplers = json.samplers.map((sampler) => structuredClone(sampler));
                ...
                const textures = json.textures.map((texture) => {
                    ...
                    //this.gl.texParameteri(this.gl.TEXTURE_2D, this.gl.TEXTURE_MIN_FILTER, sampler.minFilter);
                    //this.gl.texParameteri(this.gl.TEXTURE_2D, this.gl.TEXTURE_MAG_FILTER, sampler.magFilter);
                    this.gl.texParameteri(this.gl.TEXTURE_2D, this.gl.TEXTURE_WRAP_S, this.gl.CLAMP_TO_EDGE);
                    this.gl.texParameteri(this.gl.TEXTURE_2D, this.gl.TEXTURE_WRAP_T, this.gl.CLAMP_TO_EDGE);
                    // Upload texture
                    this.gl.texImage2D(this.gl.TEXTURE_2D, 0, this.gl.RGBA, imageData.width, imageData.height, 0, this.gl.RGBA, this.gl.UNSIGNED_BYTE, imageData.imageRGBA);
                ...
                return {
                    ...
                    primitives: mesh.primitives.map((primitive) => {
                        ...
                        Object.entries(primitive.attributes).forEach((param) => {
                            ...
                            this.gl.vertexAttribPointer(location, componentCounts[accessor.type], accessor.componentType, false, bufferView.byteStride || 0, (bufferView.byteOffset || 0) + (accessor.byteOffset || 0));
                        });
                        ...
                        return {
                            ...
                            offset: (indexBufferView.byteOffset || 0) + (indexAccessor.byteOffset || 0),
                            material: {
                                folder: name,
    ...
    class Transform {
        ...
        getTransformationMatrix() {
            ...
            if (this.isDirty) {
                glMatrix.mat4.fromRotationTranslationScale(this.matrix, this.rotation, this.translation, this.scale);
                this.isDirty = false;
            }
            return this.matrix;
        }
         ...
         */
        getForward() {
            glMatrix.vec3.transformQuat(this.forward, this.globalForward, this.rotation);
            return this.forward;
        }
        /**
    ...
    class Camera {
        ...
        constructor() {
            ...
            this.near = 0.1;
            this.far = 100;
            this.fovy = glMatrix.glMatrix.toRadian(45.0);
            // Properly initialize view matrix
            glMatrix.mat4.invert(this.viewMatrix, this.transform.getTransformationMatrix());
        }
        /**
         * Return view matrix.
         *
    ...
    class Scene {
        ...
        removeComponent(component) {
            ...
            if (entityIndex != -1) {
                this.componentList[entityIndex] = this.componentList[this.componentList.length - 1];
    ...
    class Shader {
        ...
        static compileShader(glContext, shaderSourceCode, typeName) {
            ...
            let shaderTypeString = `${type == glContext.VERTEX_SHADER
                ? "Vertex"
                : type == glContext.FRAGMENT_SHADER
                    ? "Fragment"
                    : () => {
        ...
        constructor(gl, vertexShaderCode, fragmentShaderCode) {
            ...
            const fragmentShader = Shader.compileShader(gl, fragmentShaderCode, "fragment");
            ...
            let activeUniforms = gl.getProgramParameter(this.shader, gl.ACTIVE_UNIFORMS);
            ...
            this.GLENUM_TO_SETTER = {
                ...
                [gl.UNSIGNED_INT_VEC2]: (location, value) => gl.uniform2uiv(location, value),
                [gl.UNSIGNED_INT_VEC3]: (location, value) => gl.uniform3uiv(location, value),
                [gl.UNSIGNED_INT_VEC4]: (location, value) => gl.uniform4uiv(location, value),
                ...
                [gl.FLOAT_MAT2]: (location, value) => gl.uniformMatrix2fv(location, false, value),
                [gl.FLOAT_MAT3]: (location, value) => gl.uniformMatrix3fv(location, false, value),
                [gl.FLOAT_MAT4]: (location, value) => gl.uniformMatrix4fv(location, false, value),
                [gl.FLOAT_MAT2x3]: (location, value) => gl.uniformMatrix2x3fv(location, false, value),
                [gl.FLOAT_MAT2x4]: (location, value) => gl.uniformMatrix2x4fv(location, false, value),
                [gl.FLOAT_MAT3x2]: (location, value) => gl.uniformMatrix3x2fv(location, false, value),
                [gl.FLOAT_MAT3x4]: (location, value) => gl.uniformMatrix3x4fv(location, false, value),
                [gl.FLOAT_MAT4x2]: (location, value) => gl.uniformMatrix4x2fv(location, false, value),
                [gl.FLOAT_MAT4x3]: (location, value) => gl.uniformMatrix4x3fv(location, false, value),
        ...
        setUniform(name, value) {
            this.GLENUM_TO_SETTER[this.uniforms[name].type](this.uniforms[name].location, value);
        }
    }
    // Basic vertex shader
...
`;
    ...
    class Renderer {
        ...
        constructor(canvasId) {
            ...
            if (canvas == null)
                throw new Error("Canvas id is not valid");
            ...
            this.directionalLight = {
                ...
                diffuse: glMatrix.vec3.fromValues(0.5, 0.5, 0.5),
        ...
        cullAndSortPrimitives() {
            ...
            // Near face
            glMatrix.vec3.scale(frustum.nearFace.point, cam.transform.getForward(), cam.near);
            glMatrix.vec3.add(frustum.nearFace.point, frustum.nearFace.point, cam.transform.translation);
            glMatrix.vec3.copy(frustum.nearFace.normal, cam.transform.getForward());
            // Far face
            glMatrix.vec3.add(frustum.farFace.point, cam.transform.translation, frontMultFar);
            ...
            glMatrix.vec3.cross(frustum.rightFace.normal, frustum.rightFace.normal, cam.transform.getUp());
            // Left face
            ...
            glMatrix.vec3.cross(frustum.leftFace.normal, cam.transform.getUp(), frustum.leftFace.normal);
            // Top face
            ...
            glMatrix.vec3.cross(frustum.topFace.normal, cam.transform.getRight(), frustum.topFace.normal);
            // Bottom face
            ...
            glMatrix.vec3.cross(frustum.bottomFace.normal, frustum.bottomFace.normal, cam.transform.getRight());
            ...
            glMatrix.vec3.normalize(frustum.rightFace.normal, frustum.rightFace.normal);
            ...
            glMatrix.vec3.normalize(frustum.bottomFace.normal, frustum.bottomFace.normal);
            // END AI CODE
            // This function calculates how far a point is in front of the plane.
            function getSignedDistanceToPlane(face, center) {
                return glMatrix.vec3.dot(face.normal, center) - glMatrix.vec3.dot(face.normal, face.point);
            }
            ...
            for (const component of this.scene.componentList) {
                ...
                for (const primitive of mesh.primitives) {
                    ...
                    if (component.light == undefined) {
                        ...
                        glMatrix.vec3.transformMat4(center, center, component.transform.getTransformationMatrix());
                        ...
                        const maxScale = Math.max(component.transform.scale[0], component.transform.scale[1], component.transform.scale[2]);
                    ...
                    // Get the material
                    const material = this.assetManager.getMaterial(primitive.material.folder, primitive.material.index);
                    // Sort by material
                    if (material.emissiveFactor != undefined && JSON.stringify(material.emissiveFactor) != noEmissiveFactor && primitiveInstance.light != undefined) {
                        // This is a light
                    ...
                    else if (material.alphaMode == undefined || material.alphaMode == "OPAQUE") {
        ...
        render() {
            ...
            glMatrix.mat4.perspective(projection, this.scene.camera.fovy, this.gl.canvas.width / this.gl.canvas.height, this.scene.camera.near, this.scene.camera.far);
            // Create the view projection matrix.
            const viewProjection = glMatrix.mat4.create();
            glMatrix.mat4.multiply(viewProjection, projection, this.scene.camera.getViewMatrix());
            ...
            shader.setUniform("numPointLights", primitiveList.light.pointLight.length);
            ...
            for (const lightIndex in primitiveList.light.pointLight) {
                ...
                const material = this.assetManager.getMaterial(light.material.folder, light.material.index);
                shader.setUniform(`pointLights[${lightIndex}].ambient`, [material?.emissiveFactor[0] * 0.1, material?.emissiveFactor[1] * 0.1, material?.emissiveFactor[2] * 0.1]);
                shader.setUniform(`pointLights[${lightIndex}].diffuse`, material?.emissiveFactor);
                shader.setUniform(`pointLights[${lightIndex}].specular`, material?.emissiveFactor);
                shader.setUniform(`pointLights[${lightIndex}].constant`, 1);
                shader.setUniform(`pointLights[${lightIndex}].linear`, 4.5 / light.light?.lightRange);
                shader.setUniform(`pointLights[${lightIndex}].quadratic`, 75 / (light.light?.lightRange * light.light?.lightRange));
                shader.setUniform(`pointLights[${lightIndex}].position`, light.transform?.translation);
            }
            ...
            for (const lightIndex in primitiveList.light.spotLight) {
                ...
                const material = this.assetManager.getMaterial(light.material.folder, light.material.index);
                shader.setUniform(`spotLights[${lightIndex}].ambient`, [material?.emissiveFactor[0] * 0.1, material?.emissiveFactor[1] * 0.1, material?.emissiveFactor[2] * 0.1]);
                shader.setUniform(`spotLights[${lightIndex}].diffuse`, material?.emissiveFactor);
                shader.setUniform(`spotLights[${lightIndex}].specular`, material?.emissiveFactor);
                shader.setUniform(`spotLights[${lightIndex}].constant`, 1);
                shader.setUniform(`spotLights[${lightIndex}].linear`, 4.5 / light.light?.lightRange);
                shader.setUniform(`spotLights[${lightIndex}].quadratic`, 75 / (light.light?.lightRange * light.light?.lightRange));
                shader.setUniform(`spotLights[${lightIndex}].position`, light.transform?.translation);
                shader.setUniform(`spotLights[${lightIndex}].direction`, light.light?.direction);
                shader.setUniform(`spotLights[${lightIndex}].cutOff`, light.light?.cutOff);
                shader.setUniform(`spotLights[${lightIndex}].outerCufOff`, light.light?.outerCutOff);
            }
            ...
            for (const primitive of solids) {
                shader.setUniform("model", primitive.transform.getTransformationMatrix());
                shader.setUniform("normalMatrix", primitive.transform.getNormalMatrix());
                const material = this.assetManager.getMaterial(primitive.material.folder, primitive.material.index);
                shader.setUniform("pbr_material.albedo", material?.pbrMetallicRoughness.baseColorFactor);
                shader.setUniform("pbr_material.metalness", material?.pbrMetallicRoughness.metallicFactor ?? 0);
                shader.setUniform("pbr_material.roughness", material?.pbrMetallicRoughness.roughnessFactor ?? 0);
                this.gl.bindVertexArray(primitive.vao);
                this.gl.drawElements(primitive.mode, primitive.count, primitive.type, primitive.offset);
                this.gl.bindVertexArray(null);
            }
            // Textured primitives
            ...
            for (const folder in textures) {
                for (const index in textures[folder]) {
                    ...
                    this.gl.bindTexture(this.gl.TEXTURE_2D, this.assetManager.getTexture(folder, Number(index)));
            ...
            for (let primitive of primitiveList.light.pointLight) {
                if ('lightTexture' in primitive) {
                    ...
                    this.gl.activeTexture(this.gl.TEXTURE0);
                    this.gl.bindTexture(this.gl.TEXTURE_2D, this.assetManager.getTexture(primitive.lightTexture.folder, primitive.lightTexture.index));
                    shader.setUniform('textureMap', 0);
                } else {
                    shader.setUniform('isTexture', false);
                    shader.setUniform("emissiveFactor", this.assetManager.getMaterial(primitive.material.folder, primitive.material.index)?.emissiveFactor);
                }
